...
% BENDING MECHANICAL TESTS IN THE TIBIA IN 4 POINT ONLY

% The program reads binary image files from Skyscan CT BMP ROIs to calculate
...
% Geometric files should be named "ID#_RT_slice_#.bmp" or "ID#_LT_slice_#.bmp".
% An example is 107_RT_slice_1.bmp.
...
% names are written as "ID#_RT" or "ID#_LT". For tibiae, the assumption is that
% bending was about the AP axis with the medial surface in tension.
%  
...
%                                                                         *
L = 8.98;           %span between bottom load points (mm)                 *
a = 3.00;           %distance between outer and inner load points (mm)    *   
compliance = 0;     %system compliance (microns/N)                        *
side = 'LT';        %input 'LT' for left and 'RT' for right tibia         *
slices = 7;         %number of slices from uCT to read in                 *
res = 17.29;        %voxel resolution in um of uCT                        *
...
if strcmp(side,'LT') == 0 && strcmp(side,'RT') == 0
...
clearvars -except L a compliance side slices res ang smoothing zzz ppp
...
for j=1:slices
    ...
    %each anatomic direction.  A and P are not dependent on wheter this is
   ...
   %Convert all pixel values to um and plot
    ...
    %Calculate extreme fiber in medial direction in um
...
Iuu = geom_ave(1);              %Centroidal MOI about the AP axis in um^4
Iuu = Iuu * 1e-12;             %Centroidal MOI about the AP axis in mm^4     
I = Iuu;                       %CentroidalMoment of Inertia about AP axis (mm^4)
 
medial_extreme = geom_ave(2);   %in um
c = medial_extreme;            %in microns 
...
%Convert the corrected load/displacement data to stress/strain using
%equations for 4 point bending
...
xlswrite('tibial_mechanics.xls', resultsxls, 'Data', rowcount)
xlswrite('tibial_mechanics.xls', headers, 'Data', 'A1')
